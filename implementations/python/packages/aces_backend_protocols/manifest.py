...
def backend_manifest_v1_model(manifest: BackendManifest) -> BackendManifestModel:
    """Render a backend manifest as the legacy v1 contract model."""

    return BackendManifestModel(
        name=manifest.name,
        provisioner={
...
def backend_manifest_v2_model(manifest: BackendManifest) -> BackendManifestV2Model:
    """Render a backend manifest as the shared v2 apparatus contract model."""
    ...
    return BackendManifestV2Model(
        ...
        compatibility=ApparatusCompatibilityModel(
            ...
            participant_implementations=sorted(manifest.compatible_participant_implementations),
        ),
        realization_support=[
            RealizationSupportDeclarationModel(
...
def backend_manifest_payload(
    manifest: BackendManifest,
    *,
    version: Literal["v1", "v2"] = "v2",
) -> dict[str, Any]:
    """Render a backend manifest as JSON-ready data for the requested contract version."""

    if version == "v1":
        return backend_manifest_v1_model(manifest).model_dump(mode="json")
